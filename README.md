...
```bash
isogenyguard generate --count 500 --out data.csv
isogenyguard analyze --input data.csv --out report.json
isogenyguard ond-art --input data.csv --out ond_art_report.json
isogenyguard ond-art --input data.csv --baseline-synthetic --out ond_art_report.json
isogenyguard ond-art --input data.csv --baseline-synthetic auto --out ond_art_report.json
isogenyguard ond-art --input data.csv --baseline-synthetic 1000 --out ond_art_report.json
isogenyguard ond-art --input data.csv --out ond_art_report.json --out-format jsonl --append
isogenyguard ond-art --input data.csv --out ond_art_report.csv --out-format csv
isogenyguard ond-art-fixture --out tests/fixtures/ond_art_report_example.json
isogenyguard ond-art-fixture --out tests/fixtures/ond_art_report_example.json --overwrite
```

OND-ART reports are validated against `isogenyguard/schemas/ond-art-report-0.1.schema.json` (disable with `--no-validate`).
...
uruz = signatures_to_uruz(wallet_signatures, SECP256K1_ORDER)
points = point_cloud_from_uruz(uruz, SECP256K1_ORDER, embedding="torus")
betti_result = check_betti_numbers(points, n_expected=2)
